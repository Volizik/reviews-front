                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} className={classes.titleWrap}>
                        ...
                        {userId === review.creatorId && (
                            <>
                                <Link to={`edit/${review.id}`} className={classes.link}>
                                    <Button color='primary' title='Изменить'><EditIcon /></Button>
                                </Link>
                                <AlertDialog
                                    title='Удаление отзыва'
                                    text='Вы действительно хотите удалить отзыв о своем сотруднике?'
                                    buttonComponent={(setIsOpen) => (
                                        <Button color='primary' title='Удалить' onClick={() => setIsOpen(true)}><DeleteIcon /></Button>
                                    )}
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={8}>
                        <Typography variant="h5" gutterBottom>
                            <strong>Адрес проживания:</strong> {review.worker.livingCountry}, {review.worker.livingCity}
                        </Typography>
                        <Typography variant="h5" gutterBottom>
                            <strong>Место работы:</strong> {review.worker.workingPlace} {review.worker.workingPosition}
                        </Typography>
